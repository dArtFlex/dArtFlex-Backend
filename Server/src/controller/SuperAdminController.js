	...
	try {
		...
		if (users.length == 0)
			return response
				...
				.send(`Not exisit user`);

		if (users[0].role != "super_admin")
			return response
				.status(HttpStatusCodes.BAD_REQUEST)
				.send(`The caller is not admin role`);
		...
		try {
			...
			if (promotions.length == 0) {
				...
				return response
					...
					.send(`Item already exisit`);
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error Add promotion, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get Admin`);
	}
};
	...
	try {
		...
		try {
			...
			if (promotions.length == 0)
				return response
					.status(HttpStatusCodes.BAD_REQUEST)
					.send(`Item not exisit in the promotion table`);
			else {
				await knex("promotion").where("item_id", itemId).del();
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`Item successfully removed from promotion table`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
	...
	try {
		...
		try {
			...
			if (_users.length == 0)
				...
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`User successfully banned`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error ban User, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
	...
	try {
		...
		try {
			...
			if (_users.length == 0)
				...
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`User successfully unbanned`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error Unban user, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
	...
	try {
		...
		try {
			...
			if (items.length == 0)
				return response
					.status(HttpStatusCodes.BAD_REQUEST)
					.send(`Not exisit item`);
			else {
				await knex("item").where("id", itemId).update({ ban: true });
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`Item successfully banned`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error Ban Item, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
	...
	try {
		...
		try {
			...
			if (items.length == 0)
				...
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`Item successfully unbanned`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error unban Item, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
	...
	try {
		...
		try {
			...
			if (_users.length == 0)
				...
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`User successfully burned`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error burn user, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
	...
	try {
		...
		try {
			...
			if (items.length == 0)
				...
				return response
					.status(HttpStatusCodes.ACCEPTED)
					.send(`Item successfully burned`);
			}
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error burn item, ${err}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
