...
const getByUser = async (request, response) => {
	...
	try {
		...
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get by user id, ${err}`);
	}
};

const getAlbumImage = async (request, response) => {
	const name = request.params.name;
	const imageData = fs.readFileSync(`./images/${name}.jpg`);
...
const createImage = async (request, response) => {
	...
	const tmpFileName =
		...
		"_" +
		new Date().getTime();
	const path = Path.resolve(__dirname, "../../images/", `${tmpFileName}.jpg`);
	...
	try {
		...
		if (result.length >= 20) {
			return response
				...
				.send("There are already 20 images in the Album");
		}
		const id = await knex("album")
...
const deleteImage = async (request, response) => {
	...
	try {
		...
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error delete image from album, ${err}`);
	}
};
module.exports = {
	getById,
