...
const getById = async (request, response) => {
	...
	try {
		...
		return response
			...
			.send(`Error Get bid list by order Id, ${err}`);
	}
};
...
const getByMarketId = async (request, response) => {
	...
	try {
		...
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get bid list by market Id, ${err}`);
	}
};
...
const getOfferByItemId = async (request, response) => {
	...
	try {
		const items = await knex("bid")
			.where("item_id", id)
			.andWhere("status", "like", "%offer%")
			.select("*");
		response.status(HttpStatusCodes.ACCEPTED).send(items);
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get bid offer by item Id, ${err}`);
	}
};
...
const getByUserId = async (request, response) => {
	...
	try {
		...
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get bid list by user Id, ${err}`);
	}
};
	...
	if (bidData.length > 0)
		...
		try {
			...
			response
				...
				.send(`List item Successfuly, id: ${id}`);
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error List Item, ${err}`);
		}
	}
};
...
const unListItem = async (request, response) => {
	...
	if (result.length == 0)
		...
		try {
			...
			response
				.status(HttpStatusCodes.CREATED)
				.send(`unList item Successfuly, id: ${id}`);
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error unList Item, ${err}`);
		}
	}
};
...
const changePrice = async (request, response) => {
	...
	if (result.length == 0)
		...
		try {
			...
			await knex("bid")
				.where("item_id", id)
				.andWhereNot("status", "listed")
				.del();
			await knex("marketplace")
				.update("start_price", newPrice)
				.where("id", result[0].id);
			await knex("bid")
				.where("market_id", result[0].id)
				.andWhere("status", "listed")
				.update("bid_amount", newPrice);
			const activityId = await knex("activity")
				.insert({
					from: creatorData[0]["user_id"],
					to: 0,
			...
			response
				.status(HttpStatusCodes.CREATED)
				.send(`item price chagned Successfuly, item_id: ${id}`);
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error unList Item, ${err}`);
...
const placeBid = async (request, response) => {
	...
	if (result.length == 0)
		return response
			.status(HttpStatusCodes.BAD_REQUEST)
			.send("marketplace id is invalid");
	else {
		if (result[0].sold)
			return response
				.status(HttpStatusCodes.BAD_REQUEST)
				.send("item is already sold");
		else {
			...
			if (!startPrice.lte(_bidAmount))
				return response
					.status(HttpStatusCodes.BAD_REQUEST)
					.send("invalid bid amount");
			if (
				!(
					result[0]["start_time"] <= getCurrentTime() &&
					result[0]["end_time"] >= getCurrentTime()
				)
			)
				return response
					.status(HttpStatusCodes.BAD_REQUEST)
					.send("invliad bid time");
		}
		...
		if (highestBid.length > 0) {
			...
			if (_bidAmount.lte(_highestBid))
				return response
					.status(HttpStatusCodes.BAD_REQUEST)
					.send(`Bid amount is less than highest bid`);
			// Withdraw previous bidder
			await knex("bid")
				.where("id", highestBid[0]["id"])
		...
		try {
			...
			if (result[0]["end_time"] - getCurrentTime() <= 900000) {
				await knex("marketplace")
					.where("id", marketId)
					.update({ end_time: parseInt(result[0]["end_time"]) + 900000 });
			}
			const activityId = await knex("activity")
				.insert({
					from: userId,
			...
			response
				.status(HttpStatusCodes.CREATED)
				.send(`Bid Placed Successfuly, id: ${id}`);
		} catch (err) {
			return response
				.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
				.send(`Error Place Bid, ${err}`);
		}
	}
};
...
const makeOffer = async (request, response) => {
	...
	if (result.length > 0)
		return response
			.status(HttpStatusCodes.BAD_REQUEST)
			.send("Item is currently listed in the market place");
	...
	try {
		const currentOffer = await knex("bid")
			.where("item_id", itemId)
			.andWhere("status", "offered")
			.select("*")
			.limit(1);
		if (currentOffer.length > 0) {
			...
			if (_bidAmount.lte(_currentOffer))
				return response
					.status(HttpStatusCodes.BAD_REQUEST)
					.send(`Offer amount is less than current offer`);
			// Withdraw previous bidder
			await knex("bid")
				.where("id", currentOffer[0]["id"])
				.update({ status: "canceled offer" });
		}
		// Insert new bid
		const id = await knex("bid").insert(data).returning("id");
		const activityId = await knex("activity")
		...
		response
			.status(HttpStatusCodes.CREATED)
			.send(`Bid offered Successfuly, id: ${id}`);
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error make offer, ${err}`);
	}
};
...
const withdrawBid = async (request, response) => {
	...
	try {
		...
		const prevHighestBid = await knex("bid")
			.where("market_id", data[0]["market_id"])
		...
		if (prevHighestBid.length > 0) {
			await knex("bid")
				.where("id", prevHighestBid[0]["id"])
				.update({ status: "pending" });
			await knex("marketplace")
				.where("id", data[0]["market_id"])
				.update({ current_price: prevHighestBid[0]["bid_amount"] });
		} else {
			await knex("marketplace")
				.where("id", data[0]["market_id"])
...
const withdrawOffer = async (request, response) => {
	...
	try {
		const data = await knex("bid").where("id", id).returning("*");
		const prevHighestOffer = await knex("bid")
		...
		if (prevHighestOffer.length > 0) {
			await knex("bid")
				.where("id", prevHighestOffer[0]["id"])
...
const claimNFT = async (request, response) => {
	...
	try {
		...
		await knex("item")
			.where("id", itemId)
			.update({ owner: buyerId, lazymint: false, lock: false });
		...
		return response
			.status(HttpStatusCodes.CREATED)
			.send(`Claim NFT successfuly`);
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error claim NFT, ${err}`);
	}
};
