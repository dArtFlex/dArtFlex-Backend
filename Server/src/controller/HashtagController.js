...
const getById = async (request, response) => {
	...
	try {
		...
		return response
			...
			.send(`Error Get hashtag by Id, ${err}`);
	}
};
...
const getByName = async (request, response) => {
	...
	try {
		...
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get hashtag by name, ${err}`);
	}
};
...
const getAll = async (request, response) => {
	try {
		...
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Get all hashtag, ${err}`);
	}
};
	...
	try {
		...
		if (result.length > 0)
			return response
				...
				.send(`hashtag exisit, id: ${result[0].id}`);
		else {
			const id = await knex("hashtag").insert(data).returning("id");
			response
				.status(HttpStatusCodes.CREATED)
				.send(`hashtag Added Successfuly, id: ${id}`);
		}
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error Create hashtag, ${err}`);
	}
};
...
const update = async (request, response) => {
	...
	try {
		...
		response
			.status(HttpStatusCodes.CREATED)
			.send(`Hashtag Updated Successfuly`);
	} catch (err) {
		return response
			.status(HttpStatusCodes.INTERNAL_SERVER_ERROR)
			.send(`Error update hashtag, ${err}`);
	}
};
