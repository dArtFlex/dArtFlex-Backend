...
const save_results = async function({ imgs, vectors, labels, size, parent1=null, parent2=null}) {
    const insert = []
    ...
    if (secrets.local_images) {
        ...
        const uploads = []
        for (let i = 0; i < imgs.length; i++) {
            const buf = Buffer.from(imgs[i].replace(/^data:image\/\w+;base64,/, ""),'base64')
            uploads.push(s3.upload({
                Bucket: bucket,
                Key: `${insert[i].key}.jpeg`,
                Body: buf,
                ACL: 'public-read',
